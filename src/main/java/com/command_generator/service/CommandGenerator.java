...
public class CommandGenerator {
    ...
    public String generate(EnchantmentRequest request) {
        ...
        if (request.getSharpness() != null && request.getSharpness() > 0) {
            enchantments.append("\"sharpness\":").append(request.getSharpness()).append(",");
        }
        if (request.getSmite() != null && request.getSmite() > 0) {
            enchantments.append("\"smite\":").append(request.getSmite()).append(",");
        }
        if (request.getBaneOfArthropods() != null && request.getBaneOfArthropods() > 0) {
            enchantments.append("\"bane_of_arthropods\":").append(request.getBaneOfArthropods()).append(",");
        }
        if (request.getKnockback() != null && request.getKnockback() > 0) {
            enchantments.append("\"knockback\":").append(request.getKnockback()).append(",");
        }
        if (request.getFireAspect() != null && request.getFireAspect() > 0) {
            enchantments.append("\"fire_aspect\":").append(request.getFireAspect()).append(",");
        }
        if (request.getLooting() != null && request.getLooting() > 0) {
            enchantments.append("\"looting\":").append(request.getLooting()).append(",");
        }
        if (request.getSweepingEdge() != null && request.getSweepingEdge() > 0) {
            enchantments.append("\"sweeping_edge\":").append(request.getSweepingEdge()).append(",");
        }
        if (request.getUnbreaking() != null && request.getUnbreaking() > 0) {
            enchantments.append("\"unbreaking\":").append(request.getUnbreaking()).append(",");
        }

        if (enchantments.length() > 0) {
            enchantments.deleteCharAt(enchantments.length() - 1); // 마지막 쉼표 제거
        }
        String id = swordType.getMinecraftId().replace("minecraft:", "");

        return String.format("/give @p %s[enchantments={%s}]", id, enchantments.toString());
    }
